...
import com.example.authorizationapi.dto.UserRegisterDto;
import com.example.authorizationapi.model.*;
...
import com.example.authorizationapi.utils.exceptions.UserDontExistException;
import com.example.authorizationapi.utils.exceptions.UserExistingWithEmail;
import com.example.authorizationapi.utils.exceptions.UserExistingWithUsername;
...
public class UserService {
    ...
    public ResponseEntity<?> loginByToken(HttpServletRequest request, HttpServletResponse response){
        try {
            ...
            log.info("Login from JWT token: " + login);
    ...
    public void register(UserRegisterDto userRegisterDTO) throws UserExistingWithUsername,UserExistingWithEmail{
        userRepository.findUserByLogin(userRegisterDTO.getLogin()).ifPresent(value->{
            log.info("Users alredy exist with this name");
            throw new UserExistingWithUsername("Users alredy exist with this name");
        });
        userRepository.findUserByEmail(userRegisterDTO.getEmail()).ifPresent(value->{
            log.info("Users alredy exist with this mail");
            throw new UserExistingWithEmail("Users alredy exist with this mail");
        });
        User user = new User();
        user.setLock(true);
    ...
    public ResponseEntity<?> login(HttpServletResponse response, User authRequest) {
        ...
        if (user != null) {
            ...
            log.info("Auth request password: " + authRequest.getPassword());
